...
<form class="user withdraw" method="POST">
	...
	<div class="form-group">
		...
		<input id="amount" type="text" class="form-control amount_input_error" name="amount">

		<span class="invalid-feedback amount_text_error" role="alert">
			<strong></strong>
		</span>
	</div>
...
<script>
	$(document).ready(function() {
		...
		var fee = val * "{{ $card_investor_fee }}" / 100;
		$('.fee_withdraw').html(fee + " $");
		}
		else{
		var fee = val * "{{ $investor_fee }}" / 100;
		$('.fee_withdraw').html(fee + " $");
		}
		});
		...
		$('.to-withdraw').on('click', function() {
			$.ajax({
				url: "{{ route('investor_to_withdraw') }}",
				type: 'post',
				data: {
					'amount': $('#amount').val(),
					'wallet': $('#wallet').val(),
		...
		$('.go-withdraw').on('click', function() {
			$.ajax({
				url: "{{ route('investor_withdraw_go') }}",
				type: 'post',
				data: {
					'code': $('#code').val()
				},
